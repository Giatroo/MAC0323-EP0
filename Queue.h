...
void Queue<T>::addIn(Queue_Iterator<T> *it, T e) {
	...
	std::cout << "it.curNode " << (*it).curNode << std::endl;
	std::cout << "it.curNode->next " << (*it).curNode->nextNode << std::endl;
	std::cout << "it.curNode->next->prevNode " << (*it).curNode->nextNode->prevNode
	          << std::endl;
