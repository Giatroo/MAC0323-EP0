...
void initAirCompanies() {
	...
	if (!cfg.is_open())
		throw MyException(ErrorTypes[FILE_NOT_FOUND], "initAirCompanies",
		                  "File " + COMPANIES_CFG_PATH + " could not be open.");

	while (cfg >> name >> numDestinations && numExistingCompanies < MAX_COMPANIES)
		existingCompanies[numExistingCompanies++] =
		    AirCompany(name, numDestinations);
